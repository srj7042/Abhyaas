...
export default function ResetPasswordPage() {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setMessage("Your password was updated successfully! Redirecting you...")
      setTimeout(() => {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-6 bg-background text-foreground font-sans transition-colors duration-200">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <p className="text-slate-400 text-sm mt-2">Enter your new security password</p>
        </div>
        ...
        <form onSubmit={handlePasswordReset} className="space-y-4 relative z-10">
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">Confirm New Password</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
              <input 
                type="password" 
